    ...
    if(inherits(modelObject, "lmerMod")){
      ...
      f <- function(i){
        if (allRepUnits[i] %in% repUnitsUsed){
          ...
          as.matrix(-t(Xi) %*% solve(Vi) %*% (Yi - Xi %*% beta))
        } else {
          matrix(rep(0,ncol(X)),ncol=1)
        }
      }
      ...
      if (inherits(modelObject, "lme")){
        ...
        numObsUsed <- ifelse(inherits(modelObject, "coxph"), modelObject$n, 
                             ifelse(inherits(modelObject,"nls"), length(predict(modelObject)), 
                                    ifelse(inherits(modelObject,"drc"), length(predict(modelObject)), 
                                           nrow(modelObject$model))))
        db <- deltab(modelObject,g)
